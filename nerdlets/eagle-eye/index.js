...
const LS_PROVIDERS_KEY = 'eagle-eye:providers';
const LS_CONFIG_KEY = 'eagle-eye:config';
const LS_INFRA_STATES_KEY = 'eagle-eye:infra-states';
...
const lsRead = (key) => {
  try {
    ...
    console.error('[EagleEye] localStorage read error:', key, e);
    return null;
  }
};
...
const lsWrite = (key, value) => {
  try {
    ...
    console.error('[EagleEye] localStorage write error:', key, e);
    throw e;
  }
};

const persistProviders = async (newProviders) => {
  lsWrite(LS_PROVIDERS_KEY, { providers: newProviders });
  return newProviders;
};

const persistInfraStates = async (infraStates) => {
  try {
    lsWrite(LS_INFRA_STATES_KEY, { infraStates });
  } catch (e) {
...
const SERVICE_QUERIES = {
  ...
  aws_ec2: (p) => {
    // EC2 sub-list is handled by Ec2InstanceList (tf_resources.json), not NRQL facet.
    return null;
  },
...
const formatCostLabel = (cost) => '₹' + Math.round(cost).toLocaleString('en-IN');
...
const worstStatus = (statuses) => {
  ...
  return statuses.reduce((worst, current) => {
    const cp = priority[current] ?? -1;
    const wp = priority[worst] ?? -1;
  ...
  if (!provisioned || cost === null) {
    return (
      <span className="status-badge status-badge--grey">
        <span className="status-badge__dot" />Billing
      </span>
    );
  }
            ...
            {providerId === 'aws' && (
              <div>
                ...
                >
                  ...
                  {costChartData && (
                    <button
                      onClick={e => { e.stopPropagation(); downloadCostPDF(costChartData.services, costChartData.total); }}
                      style={{ marginLeft: 'auto', fontSize: 10, fontWeight: 600, color: '#a78bfa', background: 'none', border: 'none', padding: 0, cursor: 'pointer' }}
                    >
                      ↓ Download
                    </button>
                  )}
                </div>
                {showCostChart && <AwsBillingServiceBreakdown onDataReady={setCostChartData} />}
              </div>
            )}
          </div>
        )}
...
const downloadCostPDF = (services, total) => {
  const now = new Date();
  const monthYear = now.toLocaleDateString('en-IN', { month: 'long', year: 'numeric' });
  const fileName = `AWS-Cost-by-Service-${now.toLocaleDateString('en-IN', { month: 'long' })}-${now.getFullYear()}.pdf`;
  ...
  const generate = () => {
    ...
    doc.text('AWS Cost by Service', 14, 14);
    doc.setFontSize(9); doc.setFont('helvetica', 'normal'); doc.setTextColor(200, 180, 255);
    doc.text(`${monthYear}  |  Eagle Eye  |  Tibil Solutions`, 14, 23);
    ...
    doc.text('SERVICE', 17, y);
    doc.text('COST (INR)', 160, y, { align: 'right' });
    ...
    services.forEach((svc, i) => {
      ...
      if (i % 2 === 0) { doc.setFillColor(250, 250, 252); doc.rect(14, y - 4, 182, 8, 'F'); }
      doc.setFontSize(10); doc.setTextColor(31, 41, 55);
      doc.text(svc.name, 17, y);
      doc.text(inr(svc.cost), 160, y, { align: 'right' });
      doc.setFontSize(9); doc.setTextColor(107, 114, 128);
      doc.text((svc.cost / total * 100).toFixed(1) + '%', 195, y, { align: 'right' });
      y += 8;
    });

    // Total row
    y += 2;
    doc.setDrawColor(209, 213, 219); doc.line(14, y, 196, y); y += 6;
    doc.setFontSize(11); doc.setFont('helvetica', 'bold'); doc.setTextColor(31, 41, 55);
    doc.text(`Total (${services.length} services)`, 17, y);
    doc.text(inr(total), 160, y, { align: 'right' });
    doc.setFontSize(9); doc.setTextColor(107, 114, 128);
    doc.text('100%', 195, y, { align: 'right' });

    // Footer
    doc.setFont('helvetica', 'normal'); doc.setFontSize(8); doc.setTextColor(156, 163, 175);
    doc.text(`Generated by Eagle Eye on ${now.toLocaleString('en-IN')}`, 14, 287);
...
const ServiceCostPieChart = ({ query, rateLabel = null, onDataReady = null }) => {
  ...
  return (
    <NrqlQuery accountIds={[ACCOUNT_ID]} query={query} pollInterval={300000}>
      {({ data, loading }) => {
        ...
        if (onDataReady) {
          const key = total.toFixed(0);
          if (notifiedRef.current !== key) {
            notifiedRef.current = key;
            setTimeout(() => onDataReady({ services, total }), 0);
          }
        }
        return (
          <div style={{ padding: '8px 12px 14px' }}>
            ...
            <div style={{ marginTop: 5, borderTop: '1px solid var(--ee-row-div)', paddingTop: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--ee-t1)' }}>Total</span>
...
const AwsBillingServiceBreakdown = ({ onDataReady = null }) => {
  ...
  return <ServiceCostPieChart query={query} onDataReady={onDataReady} />;
};
...
const ProjectsRendered = ({ provider, allResults, billingCost, onManage, onInfraAction, onNotify, infraStates, onLifecycleChange }) => {
  ...
  const projectStatuses = provider.projects.map((p, i) => {
    ...
    const deduped = deduplicateResourceStatuses(r.resourceStatuses);
    const nonUnknown = deduped.filter(rs => rs.status !== 'unknown');
  ...
  provider.projects.forEach((p, i) => {
    ...
    if (s === 'billing_only') {
      projectHealthMap[p.name] = billingCostToStatus(billingCost);
    } else {
      projectHealthMap[p.name] = s ?? 'unknown';
    }
  });
  ...
  const billStatus = anyProvisioned ? billingCostToStatus(billingCost) : 'unknown'; // eslint-disable-line no-unused-vars
  ...
  const resourcePillStatus = live.length > 0 ? cloudStatus : 'unknown';
  const billingPillStatus = (provider.id === 'gcp' && gcpBillingNotConfigured)
    ? 'unknown'
    : billingCostToStatus(billingCost);

  const pillWorst = (() => {
    const candidates = [resourcePillStatus, billingPillStatus].filter(s => s !== 'unknown');
    if (candidates.length === 0) return 'unknown';
    return worstStatus(candidates);
  })();
  const cardMeta = STATUS_META[pillWorst] ?? STATUS_META.unknown;
...
const EagleEye = () => {
  ...
  const toggleTheme = () => {
    ...
    setTheme(next);
    try { localStorage.setItem('eagle-eye:theme', next); } catch {}
  };
  ...
  const handleSaveToken = async (token) => {
    lsWrite(LS_CONFIG_KEY, { accessToken: token });
    setGhToken(token);
  };

  const handleRemoveToken = async () => {
    lsWrite(LS_CONFIG_KEY, { accessToken: '' });
    setGhToken('');
  };
