...
// ─── localStorage helpers (replacing NerdStorage) ─────────────────────────────
...
// ─── Billing components ───────────────────────────────────────────────────────
// FIX: BillingHealthBadge now accepts an optional `provisioned` prop.
// When provisioned=false (no lifecycle set for any non-billing project), the
// pill always renders grey regardless of the actual cost value — matching GCP behaviour.
const BillingHealthBadge = ({ cost, provisioned = true }) => {
  // If nothing is provisioned yet, always show grey — same as GCP "No Data"
...
const AwsTfInlineLoader = ({ project, lifecycle }) => {
  ...
  React.useEffect(() => {
    if (!loading && resources && resources.length > 0 && !savedRef.current) {
      ...
      // Read current providers from localStorage and update
...
const EagleEye = () => {
  ...
  useEffect(() => {
    ...
    // Load config (GitHub token) from localStorage
    ...
    // Load infra states from localStorage
